...
class Customer
{
     ...
     * @Assert\NotBlank(message="Le prénom du client est obligatoire")
     * @Assert\Length(
     *     min=3, minMessage="Le nom doit faire entre 3 et 255 caractéres",
    ...
    public function getUnpaidAmount(): float
    {
        return array_reduce($this->invoices->toArray(), function (float $total, Invoice $invoice) {
            return $total + (
                $invoice->getStatus() == "PAID" || $invoice->getStatus() == "CANCELLED" ? 0 : $invoice->getAmount()
            );
        }, 0);
    }
